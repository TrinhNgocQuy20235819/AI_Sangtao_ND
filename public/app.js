...
(function () {
  ...
  // 1. TẦNG DATA ĐỘC QUYỀN ONIIZ (NẠP Não bộ)
  const ONIIZ_CORE = {
    vibe: `=== QUY TẮC BẮT BUỘC (ONIIZ BRAND KNOWLEDGE) ===
1. ĐỊNH VỊ: Thương hiệu mỹ phẩm nam Gen Z. Triết lý "Lười nhưng vẫn xịn". Bán sự tự tin, không chỉ bán mỹ phẩm.
2. TONE OF VOICE (BẮT BUỘC):
   - Xưng hô: "ông/bro/anh em" và "tôi/mình". Vibe: Gần gũi như ông anh, hóm hỉnh. Cấm đọc như văn mẫu, sáo rỗng. Cấm dạy đạo lý.
   - Từ vựng cốt lõi: Bro, xịn, thơm, clean, 1 bước thôi.
3. INSIGHT KHÁCH HÀNG: Nam 18-28. Ngại skincare nhưng sợ bị chê lôi thôi. Trigger mua là do vấn đề (bết tóc/gàu/da nhờn) hoặc do bạn gái/crush phàn nàn.`,
    products: {
      'zerog': `Sản phẩm: ZeroG Shampoo. Dầu gội nam cao cấp, sạch gàu, tóc tơi nhẹ, không bết dính. Dầu gội riêng cho nam.`,
      'hyro': `Sản phẩm: Hyro Face Foam. Sữa rửa mặt tiện lợi 1 bước: sạch sâu + dưỡng B5. Dành cho người lười.`,
      'general': `Truyền thông chung về thương hiệu ONIIZ - Người bạn hoàn hảo của phái mạnh.`
    }
  };
  ...
  // FUNNEL MODAL
  // =============================================
  function setupFunnelModal() {
    const modal = $('#funnelModal');
    const btnOpen = $('#btnOpenFunnelModal');
    const btnClose = $('#btnCloseFunnelModal');
    const btnConfirm = $('#btnConfirmFunnel');
    const preview = $('#selectedFunnelPreview');
    ...
    function updatePreview() {
      const selected = Array.from(modal.querySelectorAll('.pill.selected'))
        .map((p) => {
          let color = '#cbd5e1';
          if (p.closest('.tofu')) color = '#38bdf8';
          if (p.closest('.mofu')) color = '#c084fc';
          if (p.closest('.bofu')) color = '#4ade80';
          // Render HTML safe preview
          const text = p.textContent.replace(/</g, "&lt;").replace(/>/g, "&gt;");
          return `<span style="background: rgba(15,23,42,1); border: 1px solid ${color}; color: ${color}; padding:6px 12px; border-radius:12px; font-size:0.8rem; font-weight:700;">${text}</span>`;
        });

      if (selected.length > 0) {
        preview.innerHTML = selected.join('');
      } else {
        preview.innerHTML = '<span style="color:#94a3b8; font-size:0.85rem; font-style:italic;">Chưa chọn kịch bản mồi nào...</span>';
      }
    }
  ...
  function collectWriteFormData() {
    ...
    const productKey = getSelectedPillValues('product')[0] || 'general';
  ...
  function collectHookFormData() {
    const productKey = getSelectedPillValues('hook-product')[0] || 'general';
  ...
  function buildWritePrompt(data) {
    let prompt = `Bạn là Content Creator thực chiến của thương hiệu ONIIZ.
${ONIIZ_CORE.vibe}
...
- Sản phẩm trọng tâm: ${ONIIZ_CORE.products[data.productKey] || ONIIZ_CORE.products['general']}
...
- Nội dung phải tự nhiên, đúng giọng người Việt, không máy móc.`;
  ...
  function buildReviewPrompt(data) {
    return `Bạn là Quản lý Content của thương hiệu ONIIZ.
${ONIIZ_CORE.vibe}

=== NHIỆM VỤ ===
Đánh giá, phân tích điểm mạnh/yếu, và viết lại bản cải thiện cho nội dung dưới đây theo đúng Vibe của ONIIZ.
...
3. **📝 Ghi chú thay đổi**: Giải thích ngắn gọn những gì đã thay đổi và tại sao.`;
  ...
  function buildHookPrompt(data) {
    return `Bạn là chuyên viên sáng tạo kịch bản của thương hiệu ONIIZ.
${ONIIZ_CORE.vibe}

=== NHIỆM VỤ ===
Đề xuất ${data.count} hook/ý tưởng content cho sản phẩm dưới đây (cam kết chuẩn Vibe giật gân, tự nhiên mộc mạc của ONIIZ).
